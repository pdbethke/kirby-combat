"""``synthetic_combatant()`` — a combatant from flat stats, with no build.

PROMOTED OUT OF `tests/fixtures` 2026-09-09. It began as a migration
helper and stayed a test fixture, and then a shippable example needed it:
`enumerate_actions` refuses a flat `StatBlockCombatant` ("carries no
`hero` to read powers and skills from"), so the ONLY way to put a
combatant in a fight is a build --- and every build this project can load
needs Hero Designer's licensed template, which the repo does not ship.

That made it impossible to author a scenario anyone else could run. A
public demo of the O.K. Corral cannot use the benchmark's cast, whose
archetypes and weapons come out of the Equipment Guide; it needs nine men
written from nothing. This is how they get made.

Still a stub, and the Limitations below still hold --- it is for
scenarios and tests, not for characters anybody has paid points for.

Originally: drop-in replacement for tests that construct a flat
``Combatant`` directly.
