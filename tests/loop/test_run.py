...
from kirby_combat.loop import (
    FirstLegalChooser, InvalidChoice, PhaseSituation, TacticChooser,
    UnresolvableAction, last_side_standing, next_actor_id, registered_kinds,
...
def test_run_phase_reports_no_actor_before_an_order_is_resolved():
    ...
    result = run_phase(
        session_of(fighter("a", side="x"), fighter("b", side="y")),
...
def test_run_phase_spends_one_slot_and_deals_damage():
    enc = encounter_of(fighter("a", side="x", dex=25), fighter("b", side="y", dex=10))
...
def test_a_downed_combatant_is_skipped_not_asked():
    ...
    enc = encounter_of(
        fighter("a", side="x", dex=25), fighter("down", side="y", dex=10, stun=0),
    )
    roller = RandomRoller(seed=3)
    enc = enc.run_segment(roller=lambda: roller.roll_dice(3))
...
def test_a_choice_outside_the_menu_raises_at_the_seat():
    ...
    enc = encounter_of(fighter("a", side="x", dex=25), fighter("b", side="y"))
    roller = RandomRoller(seed=3)
    enc = enc.run_segment(roller=lambda: roller.roll_dice(3))
...
def test_the_chooser_sees_enemies_by_side_not_by_id():
    ...
    enc = encounter_of(
        fighter("a1", side="pack", dex=25), fighter("a2", side="pack", dex=24),
        fighter("b", side="loner", dex=10),
    )
    roller = RandomRoller(seed=3)
    enc = enc.run_segment(roller=lambda: roller.roll_dice(3))
...
def test_a_duel_reaches_a_decision():
    enc = encounter_of(fighter("hero", side="heroes", dex=23),
                       fighter("villain", side="villains", dex=18))
    result = run_encounter(
        enc, FirstLegalChooser(), roller=RandomRoller(seed=7),
        on_unresolvable="skip", max_turns=10,
    )
    assert result.complete
    assert result.winner in ("heroes", "villains")
    assert result.phases > 0
    over, winner = last_side_standing(result.encounter.sessions[0])
    assert (over, winner) == (True, result.winner)
...
def test_a_four_way_free_for_all_ends_with_one_fighter():
    ...
    assert result.complete
    assert result.winner is None or result.winner.startswith("solo:")


def test_the_battle_of_four_armies():
    roster = [
        fighter(f"{army}{n}", side=army, dex=20 + n)
    ...
    if result.complete:
        assert result.winner in ("red", "blue", "green", "gold", None)
    else:
...
def test_a_caller_supplied_stop_condition_replaces_last_side_standing():
    ...
    result = run_encounter(
        encounter_of(fighter("a", side="x", dex=25), fighter("b", side="y")),
        FirstLegalChooser(), roller=RandomRoller(seed=3),
        until=first_blood, on_unresolvable="skip", max_turns=5,
    )
    assert result.complete
...
def test_an_already_decided_fight_returns_before_any_phase():
    result = run_encounter(
        encounter_of(fighter("a", side="x"), fighter("b", side="y", stun=0)),
        FirstLegalChooser(), roller=RandomRoller(seed=1),
    )
    assert result.complete and result.winner == "x"
    assert result.phases == 0
...
def test_the_max_turns_guard_stops_rather_than_looping():
    ...
    result = run_encounter(
        encounter_of(fighter("a", side="x", dice=1), fighter("b", side="y", dice=1)),
...
def test_a_fight_is_deterministic_for_a_seed():
    def run():
        return run_encounter(
            encounter_of(fighter("a", side="x", dex=23), fighter("b", side="y", dex=18)),
