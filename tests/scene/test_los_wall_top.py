...
def test_standing_on_the_wall_top_sees_over_it():
    ...
    assert line_of_sight_clear(on_top, GORGON, [WALL]) is True


def test_just_below_the_wall_top_is_still_blocked():
    assert line_of_sight_clear(Position(-9.0, -4.8, 7.9), GORGON, [WALL]) is False
...
def test_rooftop_below_the_wall_top_is_still_blocked():
    ...
    assert line_of_sight_clear(rooftop, GORGON, [WALL]) is False
